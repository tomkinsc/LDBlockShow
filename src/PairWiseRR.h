#ifndef PairWiseRR_H_
#define PairWiseRR_H_

using namespace std;


int Region_PairWise_RR(In3str1v * paraFA04, Para_18 * para_18 , map <string,map <llong, vector <BaseType> > >  & SNPList, int & Flag_for_pro)
{
	...
	OUTAA<<"#Region PairWise R^2"<<endl;
	vector<string> inf;
	...
	if  ( (paraFA04->Method)==3 )
	{
		...
		if ( access(Plink.c_str(), 0) == 0 )
		{
			...
			Plink="rm -rf "+(paraFA04->InStr2)+".blocks.gz  ;  mv  "+(paraFA04->InStr2)+".plink.blocks.det  "+(paraFA04->InStr2)+".blocks  ; gzip  " + (paraFA04->InStr2)+".blocks";
			std::system(Plink.c_str());
		}
		else
		{
