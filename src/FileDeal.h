...
int PreDealPLink(In3str1v * paraFA04, Para_18 * para_18  )
{
	...
	{
		commod="  --bfile  "+(paraFA04->InPlink)+"  --allow-extra-chr    --export vcf --out  "+(paraFA04->InStr2);
	}
	else if  (  (access(map.c_str(), 0) == 0)  &&  (access(ped.c_str(), 0) == 0) )
	{
		commod="  --file  "+(paraFA04->InPlink)+"  --allow-extra-chr    --export vcf --out  "+(paraFA04->InStr2);
	}
	else
	{
