...
int LDdecay_help01(int argc, char **argv , In3str1v * paraFA04, Para_18 * para_18)
{
	...
	for(int i = 1; i < argc ; i++)
	{
		...
		else if (flag  == "InGWAS")
		{
			if(i + 1 == argc) {LogLackArg(flag); return 0;}
			i++;
			paraFA04->InGWAS=argv[i];
		}
		else if (flag  == "MerMinSNPNum")
		{
			if(i + 1 == argc) {LogLackArg(flag); return 0;}
			i++;
			paraFA04->MerMinSNPNum=argv[i];
		}
		else if (flag  == "InGFF")
		{
			if(i + 1 == argc) {LogLackArg(flag); return 0;}
			i++;
			paraFA04->InGFF=argv[i];
		}
		...
		{
			paraFA04->OutPng=true;
		}
		else if (flag  == "OutPdf")
		{
			paraFA04->OutPdf=true;
		}
		else if (flag == "MAF")
		{
			if(i + 1== argc) {LogLackArg(flag);return 0;}
	...
	if ( (paraFA04->Method) >4  || (paraFA04->Method) <0 )
	{
		cerr<<"\t\t-BlockType   should be  [1/2/3/4]"<<endl;
		return 0;
	}
	if ( (paraFA04->RR_D) > 2 || (paraFA04->RR_D) < 0 )
	{
		cerr<<"\t\t-SeleVar   should be  [1/2]"<<endl;
		cerr<<"\t\t1 for D' ; 2  for RR \n"<<endl;
		return 0;
	}
